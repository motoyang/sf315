...
public:
  ...
  std::string toString() const {
    std::stringstream ss;
    ss << "begin_index: " << beg_index_ << " end_index: " << end_index_
       << " size: " << size_ << " capacity: " << capacity_ << std::endl;
    return ss.str() +
           uvp::hex2section(uvp::bin2hex((uint8_t *)data_, capacity_));
  }

  // Return number of bytes written.
  size_t write(const char *data, size_t bytes) {
    if (data == nullptr || bytes == 0 || size_ == capacity_) {
      // size_ == capacity_ means buffer is full, so the record length
      // is longer then buffer size.
