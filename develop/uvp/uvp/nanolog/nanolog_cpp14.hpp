...
namespace nanolog
{
	...
	*/
	...
	namespace
	{
		...
		template<typename T>
    struct is_c_string : std::integral_constant<bool, std::is_same<char*, std::decay_t<T>>::value || std::is_same<const char*, std::decay_t<T>>::value>
		{
		};
    ...
    std::enable_if_t<std::is_arithmetic<Arg>::value, NanoLogLine&> operator<<(Arg arg)
    {
      encode(arg, TupleIndex < Arg, SupportedTypes >::value);
      return *this;
    }
    template < typename Arg >
    std::enable_if_t<is_c_string<Arg>::value, NanoLogLine&> operator<<(Arg arg)
    {
      encode_c_string(arg);
      return *this;
    }
...
*/
	private:
		...
		void encode0(Arg... arg)
		{
            void(std::initializer_list<bool>{
                     (*reinterpret_cast<Arg*>(buffer()) = arg, m_bytes_used += sizeof(Arg), false)...
                 });
		}
    ...
    template < typename Arg >
    std::enable_if_t<std::is_arithmetic<Arg>::value, char*> decode(std::ostream & os, char * b, Arg * dummy)
    {
    ...
    template < typename Arg >
    std::enable_if_t<std::is_same<const char*, Arg>::value, char*> decode(std::ostream & os, char * b, Arg * dummy)
    {
    ...
    template < typename Arg >
    std::enable_if_t<std::is_same<char*, Arg>::value, char*> decode(std::ostream & os, char * b, Arg * dummy)
    {
      while (*b != '\0')
      {
        os << *b;
...
*/
	...
	public:
		FileWriter(std::string const & log_directory, std::string const & log_file_name,
			...
			: m_file_count(log_file_count)
			, m_file_number(log_file_count - 1)
	...
	{
	public:
		NanoLogger(std::ostream& os)
			: m_state(State::INIT)
			, m_buffer_base(new RingBuffer(1 * 1024 * 4))
			, m_file_writer(os)
...
//	extern std::unique_ptr < NanoLogger > nanologger;

//  extern std::atomic < NanoLogger * > atomic_nanologger;

	template<typename T = StdWriter>
	struct NanoLog
	{
	private:
		std::unique_ptr<NanoLogger<T>> nanologger;
		std::atomic<NanoLogger<T>*> atomic_nanologger;
	...
	public:
		...
		*/
		...
		void initialize(NonGuaranteedLogger ngl, std::string const & log_directory, std::string const & log_file_name, uint32_t log_file_roll_size_mb, uint32_t log_file_count = UINT32_MAX)
		{
			nanologger.reset(new NanoLogger<FileWriter>(ngl, log_directory, log_file_name, log_file_roll_size_mb, log_file_count));
			atomic_nanologger.store(nanologger.get(), std::memory_order_seq_cst);
		}

		void initialize(GuaranteedLogger gl, std::string const & log_directory, std::string const & log_file_name, uint32_t log_file_roll_size_mb, uint32_t log_file_count = UINT32_MAX)
		{
			nanologger.reset(new NanoLogger<FileWriter>(gl, log_directory, log_file_name, log_file_roll_size_mb, log_file_count));
			atomic_nanologger.store(nanologger.get(), std::memory_order_seq_cst);
		}

		void initialize(std::ostream & os)
		{
			nanologger.reset(new NanoLogger<StdWriter>(os));
			atomic_nanologger.store(nanologger.get(), std::memory_order_seq_cst);
		}
	...
	// need been defined in your cpp file, such as in main.cpp as follow:
	/*
	namespace nanolog
	{
			NanoLog<StdWriter> loger;
	}
	*/
	extern NanoLog<StdWriter> loger;

} // namespace nanolog

#define NANO_LOG(LEVEL) nanolog::loger == nanolog::NanoLogLine(LEVEL, __FILE__, __func__, __LINE__)
#define LOG_INFO nanolog::loger.is_logged(nanolog::LogLevel::INFO) && NANO_LOG(nanolog::LogLevel::INFO)
#define LOG_WARN nanolog::loger.is_logged(nanolog::LogLevel::WARN) && NANO_LOG(nanolog::LogLevel::WARN)
#define LOG_CRIT nanolog::loger.is_logged(nanolog::LogLevel::CRIT) && NANO_LOG(nanolog::LogLevel::CRIT)
