...
namespace nanolog
{
	...
	*/
	...
	inline void initialize(NonGuaranteedLogger ngl, std::string const & log_directory, std::string const & log_file_name, uint32_t log_file_roll_size_mb, uint32_t log_file_count = UINT32_MAX)
	{
		nanologger.reset(new NanoLogger(ngl, log_directory, log_file_name, log_file_roll_size_mb, log_file_count));
		atomic_nanologger.store(nanologger.get(), std::memory_order_seq_cst);
	}

	inline void initialize(GuaranteedLogger gl, std::string const & log_directory, std::string const & log_file_name, uint32_t log_file_roll_size_mb, uint32_t log_file_count = UINT32_MAX)
	{
		nanologger.reset(new NanoLogger(gl, log_directory, log_file_name, log_file_roll_size_mb, log_file_count));
		atomic_nanologger.store(nanologger.get(), std::memory_order_seq_cst);
	}
} // namespace nanolog
