...
void afterFib(uvp::Work *req, int status) {
  if (status == UV_ECANCELED) {
    std::cerr << "Calculation of "
              << ((std::pair<int, long> *)req->data())->first << " cancelled."
              << std::endl;
  } else {
...
int main(int argc, char *ls[]) {
  uvp::initialize("./", "queue-cancel", 1, 3);
  ...
  sig.start(
      [&req](uvp::Signal *sig, int signum) {
        std::cout << "Signal recvived." << std::endl;
        for (int i = 0; i < FIB_UNTIL; ++i) {
          req[i].cancel();
        }
        sig->stop();
